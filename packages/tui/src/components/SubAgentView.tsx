...
export function SubAgentView({ sub, expanded }: { sub: DisplaySubAgent; expanded?: boolean }) {
  ...
  // scrollback estimator (row stacking). Sub-agent reports quote tool/model
  // output, so sanitize before it reaches the frame.
  const line = curtail(
    sanitizeTerminalText(formatSubAgentLine(sub)),
    Math.max(20, (stdout?.columns ?? 80) - 4)
  );
  return (
